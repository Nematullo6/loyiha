...
class Block
{
	...
	constructor(block:Block)
	{
		...
		this.oyin = (this.targetBlock ? this.targetBlock.oyin : 0) + 1;
		this.workingPlane = this.oyin % 2 ? 'x' : 'z';
		this.workingDimension = this.oyin % 2 ? 'width' : 'depth';
		...
		this.position.y = this.dimension.height * this.oyin;
		...
		{
			let offset = this.oyin + this.colorOffset;
		...
		this.state = this.oyin > 1 ? this.STATES.ACTIVE : this.STATES.STOPPED;
		
		// set direction
		
		this.speed = -0.1 - (this.oyin * 0.005);
